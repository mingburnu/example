...
public class BookAccessBean implements Serializable {
	...
	public Collection<BookBean> getPageBooks() throws SQLException {
		...
		try {
			String queryPageString = "SELECT  * FROM (SELECT  ROW_NUMBER() OVER (ORDER BY BOOKID)"
					+ " AS RowNum, b.BOOKID, b.TITLE, b.author, b.PRICE, b.discount, b.companyID, b.fileName, b.bookNo, bc.name FROM eBook b JOIN eBookCompany bc ON  b.companyID = bc.id )"
			...
			while (rs.next()) {
				...
				bean.setBookID(rs.getInt(2));
				bean.setTitle(rs.getString(3));
				bean.setAuthor(rs.getString(4));
				bean.setPrice(rs.getDouble(5));
				bean.setDiscount(rs.getDouble(6));
				bean.setCompanyID(rs.getInt(7));
				bean.setFileName(rs.getString(8));
				bean.setBookNo(rs.getString(9));
				bean.setCompanyName(rs.getString(10));
	...
	public int updateBook(BookBean bean, InputStream is, int sizeInBytes,
			String pathName) throws SQLException {
		int n = 0;
		PreparedStatement pStmt = null;
		Connection connection = null;
		String updateString = "UPDATE eBook SET title=?,  author=?,  price=?, companyID=?,  fileName=?, bookNo=?, CoverImage = ? WHERE bookID = ?";
		if (pathName.length() == 0) { // 不修改圖片
			n = updateBook(bean);
			return n;
		}
		try {
	...
	// 新增一筆記錄
	public int insertBook(BookBean bean, InputStream is, int size) 
	                                   throws SQLException {
		int n = 0;
		Connection connection = null;
		PreparedStatement pStmt = null;
		try {
			String inserteBook = "insert into eBook (title,  author,  price, discount, "
					+ "companyID,  fileName, bookNo, CoverImage) values (?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public int insertBook(BookBean bean, InputStream is, int size,
		...
		PreparedStatement pStmt = null;
		String inserteBook = "insert into eBook (title,  author,  price, discount, "
				+ "companyID,  image, bookNo, CoverImage) values (?, ?, ?, ?, ?, ?, ?, ?)";
